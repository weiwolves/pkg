...
func TestECDSAVerify(t *testing.T) {
	for _, data := range ecdsaTestData {
		...
		err = data.method.Verify(signing, signature, csjwt.WithECPublicKeyFromPEM(key))
...
func TestECDSASign(t *testing.T) {
	for _, data := range ecdsaTestData {
		...
		if data.valid {
			...
			sig, err := data.method.Sign(signing, csjwt.WithECPrivateKeyFromPEM(key))
			if err != nil {
