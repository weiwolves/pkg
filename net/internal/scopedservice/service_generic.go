...
func (s *Service) configByContext(ctx context.Context) (ScopedConfig, error) {
	...
	if err != nil {
		...
		// made a mistake.
		return ScopedConfig{}, errors.Wrap(err, "[scopedservice] Service.configByContext.configFromScope") // rewrite error
	}
	return scpCfg, nil
}
...
func (s *Service) ConfigByScopedGetter(scpGet config.Scoped) (ScopedConfig, error) {
	...
	if s.optionFactory != nil {
		res, ok := <-s.optionInflight.DoChan(current.String(), func() (interface{}, error) {
			if err := s.Options(s.optionFactory(scpGet)...); err != nil {
				return ScopedConfig{}, errors.Wrap(err, "[scopedservice] Options applied by OptionFactoryFunc")
			}
	...
	return sCfg, errors.Wrap(err, "[scopedservice] Options applied and finaly validation")
}
	...
	if !ok && parent.Type() == scope.Website {
		...
		if ok && pScpCfg != nil {
			...
			if err := scpCfg.isValid(); err != nil {
				return ScopedConfig{}, errors.Wrap(err, "[scopedservice] Error in Website scope configuration")
			}
