...
func newService(opts ...Option) (*Service, error) {
	...
	if err := s.Options(WithDefaultConfig(scope.DefaultTypeID)); err != nil {
		return nil, errors.Wrap(err, "[signed] Options WithDefaultConfig")
	}
	if err := s.Options(opts...); err != nil {
		return nil, errors.Wrap(err, "[signed] Options any config")
	}
	return s, nil
}
...
func (s *Service) Options(opts ...Option) error {
	...
	if s.optionAfterApply != nil {
		return errors.Wrap(s.optionAfterApply(), "[signed] optionValidation")
	}
	return nil
}
...
func (s *Service) DebugCache(w io.Writer) error {
	...
	for _, scp := range srtScope {
		...
		if _, err := fmt.Fprintf(w, "%s => [%p]=%#v\n", scp, scpCfg, scpCfg); err != nil {
			return errors.Wrap(err, "[signed] DebugCache Fprintf")
		}
	}
	return nil
}
...
func (s *Service) configByContext(ctx context.Context) (ScopedConfig, error) {
	...
	if !scopeOK {
		return ScopedConfig{}, errors.NewNotFoundf("[signed] configByContext: scope.FromContext not found")
	}
	...
	if err != nil {
		...
		// made a mistake.
		return ScopedConfig{}, errors.Wrap(err, "[signed] Service.configByContext.configFromScope") // rewrite error
	}
	return scpCfg, nil
}
...
func (s *Service) ConfigByScopedGetter(scpGet config.Scoped) (ScopedConfig, error) {
	...
	if sCfg, err := s.ConfigByScopeID(current, 0); err == nil {
		if s.Log.IsDebug() {
			s.Log.Debug("signed.Service.ConfigByScopedGetter.IsValid",
	...
	if s.optionFactory != nil {
		res, ok := <-s.optionInflight.DoChan(current.String(), func() (interface{}, error) {
			if err := s.Options(s.optionFactory(scpGet)...); err != nil {
				return ScopedConfig{}, errors.Wrap(err, "[signed] Options applied by OptionFactoryFunc")
			}
			sCfg, err := s.ConfigByScopeID(current, parent)
			if s.Log.IsDebug() {
				s.Log.Debug("signed.Service.ConfigByScopedGetter.Inflight.Do",
			...
			return sCfg, errors.Wrap(err, "[signed] Options applied by OptionFactoryFunc")
		})
		if !ok { // unlikely to happen but you'll never know. how to test that?
			return ScopedConfig{}, errors.NewFatalf("[signed] Inflight.DoChan returned a closed/unreadable channel")
		}
		if res.Err != nil {
			return ScopedConfig{}, errors.Wrap(res.Err, "[signed] Inflight.DoChan.Error")
		}
		sCfg, ok := res.Val.(ScopedConfig)
		if !ok {
			return ScopedConfig{}, errors.NewFatalf("[signed] Inflight.DoChan res.Val cannot be type asserted to scopedConfig")
		}
		return sCfg, nil
	}
	...
	if s.Log.IsDebug() {
		s.Log.Debug("signed.Service.ConfigByScopedGetter.Parent",
			log.Stringer("requested_scope", current),
			log.Stringer("requested_parent_scope", parent),
			log.Stringer("responded_scope", sCfg.ScopeID),
			log.ErrWithKey("responded_scope_valid", err),
	...
	return sCfg, errors.Wrap(err, "[signed] Options applied and finaly validation")
}
...
func (s *Service) ConfigByScopeID(current scope.TypeID, parent scope.TypeID) (scpCfg ScopedConfig, _ error) {
	...
	if !current.ValidParent(parent) {
		return scpCfg, errors.NewNotValidf("[signed] The current scope %s has an invalid parent scope %s", current, parent)
	}
	...
	s.rwmu.RUnlock()
	if ok {
		return scpCfg, errors.Wrap(scpCfg.isValid(), "[signed] Validated directly found")
	}
	if parent == 0 {
	...
	if !ok && parent.Type() == scope.Website {
		...
		if ok && pScpCfg != nil {
			...
			if err := scpCfg.isValid(); err != nil {
				return ScopedConfig{}, errors.Wrap(err, "[signed] Error in Website scope configuration")
			}
	...
	if !ok {
		...
		if ok && pScpCfg != nil {
			...
			scpCfg = *pScpCfg
			if err := scpCfg.isValid(); err != nil {
				return ScopedConfig{}, errors.Wrap(err, "[signed] error in default configuration")
			}
