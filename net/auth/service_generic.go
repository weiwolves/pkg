...
func (s *Service) configByContext(ctx context.Context) (ScopedConfig, error) {
	...
	if !scopeOK {
		return ScopedConfig{}, errors.NotFound.Newf("[auth] configByContext: scope.FromContext not found")
	}
...
func (s *Service) ConfigByScopedGetter(scpGet config.Scoped) (ScopedConfig, error) {
	...
	if s.optionFactory != nil {
		...
		if !ok { // unlikely to happen but you'll never know. how to test that?
			return ScopedConfig{}, errors.Fatal.Newf("[auth] Inflight.DoChan returned a closed/unreadable channel")
		}
		if res.Err != nil {
		...
		if !ok {
			return ScopedConfig{}, errors.Fatal.Newf("[auth] Inflight.DoChan res.Val cannot be type asserted to scopedConfig")
		}
		return sCfg, nil
	}
...
func (s *Service) ConfigByScopeID(current scope.TypeID, parent scope.TypeID) (scpCfg ScopedConfig, _ error) {
	...
	if !current.ValidParent(parent) {
		return scpCfg, errors.NotValid.Newf("[auth] The current scope %s has an invalid parent scope %s", current, parent)
	}
