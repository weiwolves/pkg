...
func (s *Service) Options(opts ...Option) error {
	...
	if s.optionAfterApply != nil {
		return errors.Wrap(s.optionAfterApply(), "[geoip] optionValidation")
	}
	return nil
}
...
func (s *Service) DebugCache(w io.Writer) error {
	...
	for _, scp := range srtScope {
		...
		if _, err := fmt.Fprintf(w, "%s => [%p]=%#v\n", scp, scpCfg, scpCfg); err != nil {
			return errors.Wrap(err, "[geoip] DebugCache Fprintf")
		}
	}
	return nil
}
...
func (s *Service) configByContext(ctx context.Context) (ScopedConfig, error) {
	...
	if !scopeOK {
		return ScopedConfig{}, errors.NewNotFoundf("[geoip] configByContext: scope.FromContext not found")
	}
	...
	if err != nil {
		...
		// made a mistake.
		return ScopedConfig{}, errors.Wrap(err, "[geoip] Service.configByContext.configFromScope") // rewrite error
	}
	return scpCfg, nil
}
...
func (s *Service) ConfigByScopedGetter(scpGet config.Scoped) (ScopedConfig, error) {
	...
	if s.optionFactory != nil {
		res, ok := <-s.optionInflight.DoChan(current.String(), func() (interface{}, error) {
			if err := s.Options(s.optionFactory(scpGet)...); err != nil {
				return ScopedConfig{}, errors.Wrap(err, "[geoip] Options applied by OptionFactoryFunc")
			}
			...
			return sCfg, errors.Wrap(err, "[geoip] Options applied by OptionFactoryFunc")
		})
		if !ok { // unlikely to happen but you'll never know. how to test that?
			return ScopedConfig{}, errors.NewFatalf("[geoip] Inflight.DoChan returned a closed/unreadable channel")
		}
		if res.Err != nil {
			return ScopedConfig{}, errors.Wrap(res.Err, "[geoip] Inflight.DoChan.Error")
		}
		sCfg, ok := res.Val.(ScopedConfig)
		if !ok {
			return ScopedConfig{}, errors.NewFatalf("[geoip] Inflight.DoChan res.Val cannot be type asserted to scopedConfig")
		}
		return sCfg, nil
	}
	...
	return sCfg, errors.Wrap(err, "[geoip] Options applied and finaly validation")
}
...
func (s *Service) ConfigByScopeID(current scope.TypeID, parent scope.TypeID) (scpCfg ScopedConfig, _ error) {
	...
	if !current.ValidParent(parent) {
		return scpCfg, errors.NewNotValidf("[geoip] The current scope %s has an invalid parent scope %s", current, parent)
	}
	...
	s.rwmu.RUnlock()
	if ok {
		return scpCfg, errors.Wrap(scpCfg.isValid(), "[geoip] Validated directly found")
	}
	if parent == 0 {
	...
	if !ok && parent.Type() == scope.Website {
		...
		if ok && pScpCfg != nil {
			...
			if err := scpCfg.isValid(); err != nil {
				return ScopedConfig{}, errors.Wrap(err, "[geoip] Error in Website scope configuration")
			}
	...
	if !ok {
		...
		if ok && pScpCfg != nil {
			...
			scpCfg = *pScpCfg
			if err := scpCfg.isValid(); err != nil {
				return ScopedConfig{}, errors.Wrap(err, "[geoip] error in default configuration")
			}
