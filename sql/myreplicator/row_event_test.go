...
func TestDecodeDecimal(t *testing.T) {
	...
	for i, tc := range testcases {
		...
		expectedDecimal, _ := null.MakeDecimalBytes([]byte(tc.Expected))
		// assert.Exactly(t,value, DecodeDecimalsEquals, pos, err, expectedDecimal, tc.ExpectedPos, tc.ExpectedErr, i)
		if tc.ExpectedErr != nil {
			assert.EqualError(t, err, tc.ExpectedErr.Error(), "Index %d", i)
			continue
		}
		assert.NoError(t, err, "Index %d\n%+v", i, err)
		assert.Exactly(t, tc.ExpectedPos, pos, "Index %d", i)
		assert.Exactly(t, expectedDecimal, value, "Index %d want: %#v Have: %#v In: %q", i, expectedDecimal, value, tc.Expected)

		// special testing case of fmt.Stringer of null.Decimal
		wantStr := tc.Expected
		wantStr = strings.TrimRight(wantStr, "0")
		if wantStr == "" {
			wantStr = "0"
		}
		if strings.HasSuffix(wantStr, ".") {
			wantStr = wantStr[:len(wantStr)-1]
		}
		assert.Exactly(t, wantStr, value.String(), "Index %d", i)
	}
}
...
func TestParseRowPanic(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Exactly(t, int32(16270), rows.Rows[0][0])
}
...
func TestParseJson(t *testing.T) {
	...
	for _, tbl := range tbls {
		...
		err = rows.decode(tbl)
		assert.NoError(t, err)
		assert.Exactly(t, null.Decimal{Precision: 1, Valid: true}, rows.Rows[0][1])
	}
	...
	for _, ltbl := range longTbls {
		...
		assert.NoError(t, err)
		assert.Exactly(t, null.Decimal{Precision: 101, Valid: true}, rows.Rows[0][1])
	}
}
...
func TestEnum(t *testing.T) {
	...
	err = rows.decode(data)
	assert.NoError(t, err)
	assert.Exactly(t, int64(1), rows.Rows[0][1])
}
...
func TestMultiBytesEnum(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Exactly(t, int64(1), rows.Rows[0][1])
	assert.Exactly(t, int64(257), rows.Rows[1][1])
}
...
func TestSet(t *testing.T) {
	...
	err = rows.decode(data)
	assert.NoError(t, err)
	assert.Exactly(t, int64(5), rows.Rows[0][1])
}
...
func TestJsonNull(t *testing.T) {
	...
	err = rows.decode(data)
	assert.NoError(t, err)
	assert.Exactly(t, []byte(``), rows.Rows[0][3])
}
